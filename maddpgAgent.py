...
class MADDPG_Agent:
    ...
    def learn(self, replays, other_agent):
        ...
        rewards = torch.from_numpy(rewards[:, [self.agent_index]]).float().to(self.device)               # [m, 1]
        dones = torch.from_numpy(dones[:, [self.agent_index]].astype(np.uint8)).float().to(self.device)  # [m, 1]
        ...
        if self.agent_index == 0:
            next_actions = torch.cat([next_actions_self, next_actions_other], dim = 1).to(self.device)
        elif self.agent_index == 1:
            next_actions = torch.cat([next_actions_other, next_actions_self], dim = 1).to(self.device)
        ...
        if self.agent_index == 0:
            action_local = torch.cat([self.actor_local(states_self),
                                      other_agent.act(states_other, actor_name = 'local', noise_bool = False)], dim = 1)
        elif self.agent_index == 1:
            action_local = torch.cat([other_agent.act(states_other, actor_name = 'local', noise_bool = False),
                                      self.actor_local(states_self)], dim = 1)
